# File: main.py
"""
Simple driver to run all Cassandra replication tests sequentially, with clear sections, emojis, and colors.
"""
import sys

from colorama import init as colorama_init
from colors import SUCCESS, FAIL, INFO, RESET, SEPARATOR
...
def main() -> None:
    ...
    log(f"{'✅' if sql_ok else '❌'}  [MAIN] SQL Test {'PASSED 🎉' if sql_ok else 'FAILED ❌'}", SUCCESS if sql_ok else FAIL)

    # --- NoSQL Replication Test ---
    log("-" * 80, SEPARATOR, True)
    log(INFO + "🔧  [MAIN] Starting NoSQL Replication Test")
    nosql_ok = nosql_test(nodes)
    log(f"{'✅' if nosql_ok else '❌'}  [MAIN] NoSQL Test {'PASSED 🎉' if nosql_ok else 'FAILED ❌'}", SUCCESS if nosql_ok else FAIL)

    # --- Disconnect/Reconnect Test ---
    log("-" * 80, SEPARATOR, True)
    log(INFO + "🔌  [MAIN] Starting Disconnect/Reconnect Test")
    disconnect_ok = disconnect_reconnect_test(nodes)
    log(f"{'✅' if disconnect_ok else '❌'}  [MAIN] Disconnect/Reconnect Test {'PASSED 🎉' if disconnect_ok else 'FAILED ❌'}", SUCCESS if disconnect_ok else FAIL)

    # --- Network-Limit Test ---
    log("-" * 80, SEPARATOR, True)
    log(INFO + "🌐  [MAIN] Starting Network-Limit Test")
    network_ok = network_limit_test(nodes)
    log(f"{'✅' if network_ok else '❌'}  [MAIN] Network-Limit Test {'PASSED 🎉' if network_ok else 'FAILED ❌'}", SUCCESS if network_ok else FAIL)
